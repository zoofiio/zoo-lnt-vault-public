...
import {ILntVaultAethirSigner} from "src/interfaces/aethir/ILntVaultAethirSigner.sol";
import {ICheckerClaimAndWithdraw} from "src/interfaces/aethir/ICheckerClaimAndWithdraw.sol";

/// @custom:oz-upgrades-from src/aethir/LntVaultAethirUpg.sol:LntVaultAethirUpg
contract LntVaultAethirUpgV2 is ILntVaultAethir, UUPSUpgradeable, ReentrancyGuardUpgradeable, TokenHelper, ERC721HolderUpgradeable {
    ...
    */
    ...
    address public aethirSignerOracle;
    address public aethirClaimAndWithdraw;

    event UpdateAethirClaimAndWithdraw(address indexed signerOracle, address indexed claimAndWithdraw);

    function updateAethirClaimAndWithdraw(address _signerOracle, address _claimAndWithdraw) external nonReentrant onlyOwner {
        require(_signerOracle != address(0) && _claimAndWithdraw != address(0));
        aethirSignerOracle = _signerOracle;
        aethirClaimAndWithdraw = _claimAndWithdraw;

        emit UpdateAethirClaimAndWithdraw(aethirSignerOracle, aethirClaimAndWithdraw);
    }

    function claimAethirRewards(        
        uint256 orderId,
        uint48 cliffSeconds,
        ...
        require(msg.sender == owner() || msg.sender == ILntVaultAethirSigner(aethirSignerOracle).signer());
        require(aethirClaimAndWithdraw != address(0));
        ICheckerClaimAndWithdraw(aethirClaimAndWithdraw).claim(
            orderId,
            cliffSeconds,
            expiryTimestamp,
            amount,
    ...
    function withdrawAethirRewards(
        ...
        require(msg.sender == owner() || msg.sender == ILntVaultAethirSigner(aethirSignerOracle).signer());
        require(aethirClaimAndWithdraw != address(0));
        ICheckerClaimAndWithdraw(aethirClaimAndWithdraw).withdraw(
            orderIdArray,
            expiryTimestamp,
            signatureArray
        );
